...
#show : official.with(
  ...
  abstract: [
    ...
    theorems that are not actually correct. For example, this may occur due to subtle mistakes in
    definitions or forgetting to state an assumption. Counterexample finding techniques can be used
    to reduce this time sink by identifying incorrect theorems before time is spent on attempting
    to prove them.
...
#[
  // HACK: show . with space after it for math notation of quantifiers in the theory section, later
  // switch off because we have stuff like $0.x$. Realistically speaking I should've solved this
  // differently but it's too late now.
...
Nitpick @nitpick and Quickcheck @quickcheck. In contrast, systems grounded in @DTT have so far
...
For such theorems, finite model finders, such as Nitpick, are more effective. Nitpick reduces the original theorem to
...
can broadly be located somewhere on a spectrum between concrete execution and fully symbolic
reasoning.

On the concrete end of the spectrum, the most widely deployed technique is
...
The main strength of this approach is the ability to cover a large part of the input space quickly: computers can generate and
evaluate many candidates within a short time span. The main weakness
are complex constraints on the input space that make it hard to generate valid inputs.
To counteract this, ACL2, Isabelle's Quickcheck, and later iterations of Rocq's QuickChick @quickchick-multirelations,
...
explored up to a small bound within a reasonable time frame. This can cause bounded exhaustive testing to
miss larger counterexamples.
...
implements a narrowing-based approach. The general idea of narrowing is to symbolically evaluate the
...
the reduction, focusing on the fragments relevant to this thesis. Additional details are discussed
...
This base calculus is extended with several axioms: quotients, function extensionality, and
choice. A thorough description of Lean's logic and its properties can be found in the works of
Carneiro @mario-type-theory and Ullrich @sebastianphd.
...
called elaborators that are also written in Lean. The mechanism for integrating additional elaborators
allows for the development of new languages entirely as extensions to Lean itself. For
...
relevant fragment for this thesis is given by the following grammar:
$
...
For this problem, Nunchaku quickly identifies the following solution:
```nun
SAT: { val m := S Z. val n := S Z. }
```

To find these solutions, Nunchaku applies long pipelines of reduction passes that
reduce to the input logics of its backend solvers. With the CVC4 pipeline alone containing 21 passes,
explaining the pipelines in full would be far out of scope for this thesis. Instead, the following
...
recursive calls. For this reason, Nunchaku creates a copy of `map`, specialized on the
...
solvers. These solvers might give up on the problem, prove that it is valid, or return a
...
$
Since this is a mere syntactic restriction of Lean's core calculus, we can reuse Lean's type
...
while the other way around both steps have to deal with it.

Dependent types occur in two flavors in the input fragment. First, a function or
inductive may take an argument that is a proof. This is an issue because Nunchaku has no notion of proof terms,
...
$
Next, I define the invariant generator $mkinv(alpha, Gamma)$. Generating invariants for
...
& "ctor"_n : (overline(z)_n : overline(gamma)_n) -> c " " overline(a) " " overline(x) " " overline(t)_n \
$
The replacement proposition $c'$ of $c$ is
$
  (overline(x) : overline(alpha'))","(overline(y) : overline(beta')) &= dentype((overline(x) : overline(alpha))","(overline(y) : overline(beta)) , (overline(a) : Type u)) \
  (overline(z) : overline(gamma')_i) &= dentype((overline(z)_i : overline(gamma)_i), (overline(a) : Type u), (p_overline(a) : overline(a) -> Prop), (overline(x) : overline(alpha))) \
  overline(t')_i &= denterm(overline(t)_i, (overline(a) : Type u), (p_overline(a) : overline(a) -> Prop), (overline(x) : overline(alpha)), (overline(z)_i : overline(gamma)_i)) \
...
definition are the addition of the predicate parameters for type arguments and dependent type
erasure:
$
...
erase all non-propositional dependent types from it via $denprop(alpha, epsilon)$. This
...
dependent type and also does not transitively contain any dependent types, which leaves nothing for
...
Here, $c$ is a trivial inductive type if it takes no parameters or indices and all of its
...
With dependent types eliminated, we are left with a logic with rank-1 polymorphism. In
...
Note that $"C"$ still has to consider the dependent arrow and term arguments to types since that
...
instantiated with each of the solutions for their type variables. However, a finite solution does
not always exist.
...
with an infinite set of types of the form $"Two" a, "Two" ("Two" a), "Two" ("Two" ("Two" a)), dots$.
This makes monomorphization for this problem infeasible, as we cannot generate infinitely many
specialized copies of $"Tree"$.

To detect these rare situations, Lutze et al. convert the constraints into a directed graph and
...
node to the origin node of each marked edge to search for such a cycle. However, this algorithm is
wasteful, as
...
$G$ to a unique identifier of the SCC it is a member of. This can be done using Tarjan's
...
Since the constraint problem is solvable, this fixpoint problem is guaranteed to admit a finite solution
set for every type variable. The types occurring in these sets are free of type variables and can
thus be called ground types:
...
Finally, the original term $"add" ("length" "Nat" x_1) " " ("length" "String" x_2)$ gets
...
If Nunchaku finds a counterexample, it is translated backwards along the pipeline steps of
...
In the encoding pipeline implementation, none of the pipeline steps ever leave Lean's own expression
representation. This has several benefits. For one, all pipeline steps can make use of Lean's
metaprogramming API for type inference, reduction, working with binders, etc. Furthermore, the Lean kernel is able to
...
for the translation. Afterwards, it preprocesses the proof state before
...
derives all the equations for the definitions involved in the current proof state.
For the most part, this is done with Lean's `Meta.getEqnsFor?` facility. However, this API does not offer equations for
...
With all the relevant information collected, the pipeline proceeds to preprocess the current proof
state in preparation for the reduction. This preprocessing step consists of three parts. First, it applies
function extensionality where possible to avoid the previously described unsoundness as
...
#pagebreak(weak: true)
```lean
-- Lean's `+` generalizes to heterogeneous addition
...
classes, this term can be simplified back to just `Nat.add a b`.
...
theorem. To avoid this, Chako replaces the local type variables with global axioms of the
...
After the preprocessing step, Chako eliminates dependent types as described in @sect_trans_dtt. The
...
The last step before producing the Nunchaku problem is monomorphization. Like the dependent
...
types or universal quantification. Finally, Chako invokes Nunchaku on the generated problem and
...
for higher-order functions to deal with them. This encoding obscures the original problem structure
and can thus make it difficult for the backend solvers to reliably solve the problems.
...
is mostly a port of the CVC4 backend with two cvc5 specific modifications. The first
...
CVC4 at some point and was thus not present in cvc5. However, the cvc5 developers kindly implemented a
replacement in the form of the, currently undocumented, `get-model-domain-elements`.#footnote[https://github.com/cvc5/cvc5/pull/12088]
...
might allow the solver to explore more approaches, increasing the chances of success. The second
...
addition, it solved two new problems on which CVC4 had given up previously. While it might not
...
During the development of Chako, I encountered several bugs along the entire reduction
...
it must be the one we were looking for. However, the current statement does not capture this idea
...
This invariant can be expressed almost directly in Lean as follows:
```lean
...
def insert [LT α] [DecidableLT α] (t : AATree α) (x : α) : AATree α :=
  ...
  | node y lvl l r =>
    if x < y then
      ...
      node y lvl l r
```
To keep the tree balanced, the `insert` function must preserve the `WF` invariant:
```lean
...
standard library. Through this, I aim to investigate three research questions:
...
Because there is no standardized data set for counterexample finding available in Lean, this
...
To investigate *RQ1* and *RQ3*, I run Chako on the already proven theorem statements.
...
any errors encountered during encoding are most likely due to the target theorem being outside the
fragment supported by Chako.

To investigate *RQ2*, I follow the evaluation approach of Nitpick and previous Isabelle
...
be too difficult. All problems were run with Chako's default wall time limit of ten
...
total. Note that the $10.3%$ of mutants that the solvers gave up on likely contain both true and
...
The main potential source of errors in this evaluation is the generation of the wrong statements.
It is unclear whether the mutation procedure generates the kinds of statements that users are
practically interested in finding counterexamples for. Furthermore, the ability of Chako to find counterexamples
can vary a lot depending on the kinds of problems it is used on. This can be observed in the comparatively low
...
typed theorem prover. While the underlying translation is limited to ML-style polymorphism for
...
functions can make use of these advanced kinds of polymorphism internally. Thus, to
...
On the solver side, many additional improvements to Nunchaku are possible. As mentioned in
@sect_impl_nunchaku, it currently suffers from inconsistencies on a somewhat regular basis. One way to prevent this in
the future would be to test it with a large and diverse set of problems, in the hope of discovering
these inconsistencies before they affect users. In addition, the authors of Nunchaku and myself
